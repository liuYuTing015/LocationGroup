...
import com.service.locationlibrary.MemoryCache
...
class MqttSend(broker: String, accessKey: String, secretKey: String) {
    ...
    fun sendMessage(topic: String, producerClientId: String, detailJson: String) = try {
        if (sampleClient == null) {
            sampleClient = MqttClient(broker, producerClientId, persistence)
            ...
            (sampleClient as MqttClient).setCallback(object : MqttCallback {
                override fun connectionLost(throwable: Throwable) {
                    System.out.println("mqtt connection lost")
                    throwable.printStackTrace()
                    while (!(sampleClient as MqttClient).isConnected) {
                        try {
                            (sampleClient as MqttClient).connect(connOpts)
